...
class Attention(nn.Module):
    def __init__(self, query_dim, context_dim = None, inner_dim = None):
        super().__init__()
...
class img_feature_extractor(nn.Module):
    def __init__(self, args):
        super(img_feature_extractor, self).__init__()

        self.backbone = resnet.resnet18(in_channels=3, pretrained=True, progress=True)
...
class feature_fusion(nn.Module):
    def __init__(self, args):
        ...
        self.model = Attention(query_dim=args.emb_dims, context_dim = 128, inner_dim=128)
        self.out = nn.Linear(256, 128, bias = False)
        nn.init.kaiming_normal_(self.out.weight, mode='fan_in', nonlinearity='relu')
    def forward(self, q, context):
        fuse_q = self.model(q, context)
        fusion_out = self.out(torch.cat((q.transpose(1,2),fuse_q),dim=2))
        return (fusion_out+fuse_q).transpose(1,2)
