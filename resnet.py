...
class ResNet_preprocess(nn.Module):

    def __init__(self, in_channels, block, layers, num_classes=1000, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None):
        super(ResNet_preprocess, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
...
def _resnet_preprocess(in_channels, arch, block, layers, pretrained, progress, **kwargs):
    model = ResNet_preprocess(in_channels, block, layers, **kwargs)
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls[arch],progress=progress)
        model.load_state_dict(state_dict)
    return model

def resnet_preprocess(in_channels=3, pretrained=False, progress=True, **kwargs):
    ...
    """
    return _resnet_preprocess(in_channels, 'resnet18', BasicBlock, [2, 2, 2, 2], pretrained, progress,
                   **kwargs)
